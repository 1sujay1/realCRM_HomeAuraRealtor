    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                onSuccess?.();
                onClose();
                setForm({ name: '', email: '', phone: '', source: 'CRM', project: '', budget: '', type: '', isJunk: false });
            } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                        <input id="isJunk" name="isJunk" type="checkbox" checked={form.isJunk} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded" />
